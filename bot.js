...
client.on('ready', ()=>{
    client.channels.cache.get('810213524864761877').join() 
    })
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, aleykumselam hg kanka.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "sa") 
    return message.channel.send(`${message.author}, Aleykum Selam hoşgeldin kanka.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "LetAnc") 
    return message.channel.send(`${message.author}, LetAnc her daim <3`)
});
//════════════════════════════════════════════════════════════════════════════════
// AFK SİSTEMİ
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        const kufur = ["amk", "aq", "oç", "sik", "çük", "yarak", "amcık", "göt", "ananı", "amq", "orospu", "orspu", "oruspu", "anani", "amcik", "sıkerım", "sıkerim", "sikerım", "yarram", "amık", "kahpe", "piç", "amsk", "seks", "sikiş", "yarrak", "gavat", "orospuçocuğu", "orospucocugu", "ananısikim", "gahpe", "kahpe", "ibne", "ipne", "pezevenk", "puşt", "fuck", "bitch", "bastard", "asshole","14fps minecraft"];
